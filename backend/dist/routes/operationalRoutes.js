...
router.post('/stock/request', async (req, res) => {
    const requestedById = req.user.id;
    const { stockItemId, quantity, notes } = req.body;
...
router.patch('/tasks/:id/undo', async (req, res) => {
    const id = parseInt(String(req.params['id'] ?? '0'));
    const updated = await prisma.task.update({
        where: { id },
...
router.post('/compliance', async (req, res) => {
    ...
    const { type, description, result, recordedAt } = req.body;
