...
router.get('/sales/by-branch', async (req: Request, res: Response) => {
  const from = new Date();
...
router.get('/inventory', async (req: Request, res: Response) => {
  ...
  const where =
    role === 'MANAGEMENT' && branchId ? { branchId } : {};
...
router.get('/inventory/requests', async (req: Request, res: Response) => {
  const role     = req.user!.role;
  const branchId = req.user!.branchId;
...
router.patch('/inventory/requests/:id', async (req: Request, res: Response) => {
  ...
  const updated = await prisma.stockRequest.update({
    where: { id },
    data:  { status: status as 'APPROVED' | 'REJECTED' | 'FULFILLED' },
  });
...
router.patch('/inventory/:id/adjust', async (req: Request, res: Response) => {
  const id = parseInt(String(req.params['id'] ?? '0'));
  const { quantity, type, notes } = req.body as {
    quantity: number; type: 'IN' | 'OUT'; notes?: string;
  };
...
router.get('/staff', async (req: Request, res: Response) => {
  ...
  const where =
    role === 'MANAGEMENT' && branchId
      ? { branchId, role: { in: ['OPERATIONAL' as const, 'MANAGEMENT' as const] } }
      : { role: { in: ['OPERATIONAL' as const, 'MANAGEMENT' as const] } };
...
router.post('/alerts', async (req: Request, res: Response) => {
  const { type, title, message, branchId } = req.body as {
    type: string; title: string; message: string; branchId?: number;
  };
  ...
  const alert = await prisma.alert.create({
    data: {
      type:     type as 'LOW_STOCK' | 'HIGH_SALES' | 'LOW_SALES' | 'COMPLIANCE_DUE' | 'STAFF_SHORTAGE' | 'SYSTEM',
      title,
      message,
...
router.post('/tasks', async (req: Request, res: Response) => {
  const { branchId: bodyBranchId, title, description, dueDate } = req.body as {
    branchId?: number; title: string; description?: string; dueDate?: string;
  };
